...
function loadPending(): PendingDownload[] {
  ...
  try {
    ...
    log(
      "Downloads",
      `Discarding unreadable ${PENDING_KEY} (${describeError(e)}); it held: ${raw.slice(0, 400)}`,
    );
    return [];
  }
...
function savePending(pending: PendingDownload[]): void {
  try {
    ...
    log(
      "Downloads",
      `Could not save ${pending.length} pending download(s) to ${PENDING_KEY}: ${describeError(e)}`,
    );
  }
}
...
export function removePendingDownload(id: string): void {
  ...
  log(
    "Downloads",
    `Cleared pending download ${id}; ${pending.length} still recorded`,
  );
}
    ...
    set((state) => {
      if (partial.status !== undefined) {
        ...
        if (current && current.status !== partial.status) {
          log(
            "Downloads",
            `"${current.name}" (${id}): ${current.status} -> ${partial.status}`,
          );
        }
      }
      return {
