import {
  ...
  AppStatusType,
  BLOCKED_ACTIONS,
} from "@renderer/scripts/AppStatus";
...
interface ProgressBarState {
  ...
  update(
    partial: Partial<
      Pick<
        ProgressBarState,
        "busy" | "currentStatus" | "message" | "progress" | "show"
      >
    >,
  ): void;
  reset(): void;
...
export class ProgressBar {
  private static state = create<ProgressBarState>((set) => ({
    ...
    setStatus(status) {
      set((state) => {
        const next = StateUtils.resolveSetStateAction(
          status,
          state.currentStatus,
        );
        if (next !== state.currentStatus)
          log("Progress", `Status: ${state.currentStatus} -> ${next}`);
    ...
    update(partial) {
      set((state) => {
        if (
          partial.currentStatus !== undefined &&
          partial.currentStatus !== state.currentStatus
        ) {
          log(
            "Progress",
            `Status: ${state.currentStatus} -> ${partial.currentStatus}`,
          );
        }
        return partial;
      });
  ...
  static useState<T>(
    selector?: (state: ProgressBarState) => T,
  ): T | ProgressBarState {
  ...
  private static enter(
    ...
    resetOptions: ProgressResetOptions,
  ): void {
    if (this.owner === null) {
