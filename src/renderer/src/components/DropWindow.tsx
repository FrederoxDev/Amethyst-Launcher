  ...
  useEffect(() => {
        ...
        for (const file of items) {
          ...
          try {
            ...
            if (st.isDirectory()) {
              ...
              log(
                "DropWindow",
                `Ignoring ${file_path}: it is neither a file nor a folder`,
              );
            ...
            log(
              "DropWindow",
              `Could not read the dropped ${file_path}: ${describeError(e)}`,
            );
    ...
    function ImportArchive(archive_path: string) {
      if (!isModArchive(archive_path)) {
        log(
          "DropWindow",
          `Rejecting ${archive_path}: its extension is not .amethyst or .zip`,
        );
        setError(
          `${path.basename(archive_path)} is not a mod file. Drop a .amethyst or .zip file.`,
        );
        return;
      }
      ...
      ImportModArchive(archive_path).catch((error) => {
        log(
          "DropWindow",
          `Importing ${archive_path} failed: ${describeError(error)}`,
        );
    ...
    function openFile(_event: unknown, file_path: string) {
      log(
        "DropWindow",
        `Opening ${file_path} handed over by the operating system`,
      );
      if (ProgressBar.isBusy()) {
        log(
          "DropWindow",
    ...
    async function ImportFolder(folder_path: string) {
      log(
        "DropWindow",
        `Copying the folder ${folder_path} into ${paths.modsPath}`,
      );
      try {
        ...
        log(
          "DropWindow",
          `Copying ${folder_path} into ${paths.modsPath} failed: ${describeError(error)}`,
        );
        setError((error as Error).message);
      }
    }
