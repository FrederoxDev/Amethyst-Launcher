...
async function downloadToTemp(
  ...
  try {
    await Downloader.downloadFile(
      url,
      filePath,
      (transferred, total) => onProgress?.(transferred, total),
      signal,
    );
    ...
    await fs.promises.rm(filePath, { force: true }).catch((cleanupError) => {
      log(
        "Recovery",
        `Could not delete the abandoned temp file ${filePath}: ${describeError(cleanupError)}`,
      );
    });
    if (isAbort(e)) {
      log(
        "Recovery",
        `Download of ${url} was cancelled by the user; ${filePath} deleted`,
      );
      return { ok: false, error: "Download cancelled" };
    }
    log(
      "Recovery",
      `Download of ${url} to ${filePath} failed: ${describeError(e)}`,
    );
...
function failRecovery(pending: PendingDownload, message: string): void {
  log(
    "Recovery",
    `Giving up on ${pending.type} "${pending.name}" (${pending.id}): ${message}`,
  );
  useAppStore
    .getState()
    .setDownloadingMods((prev) => prev.filter((n) => n !== pending.name));
  useDownloadStore
    .getState()
    .updateDownload(pending.id, { status: "error", progress: 0 });
...
async function resumeModDownload(pending: PendingDownload): Promise<void> {
  log(
    "Recovery",
    `Resuming mod download "${pending.name}" (${pending.id}) from ${pending.url}`,
  );
  ...
  if (!ok) {
    failRecovery(
      pending,
      `Could not finish downloading ${pending.name}: ${error}`,
    );
    return;
  }
  ...
  useDownloadStore
    .getState()
    .updateDownload(pending.id, { status: "extracting", progress: 1 });
  ...
  try {
    ...
    log(
      "Recovery",
      `Installing the resumed archive ${filePath} failed: ${describeError(e)}`,
    );
    ...
    await fs.promises.rm(filePath!, { force: true }).catch((cleanupError) => {
      log(
        "Recovery",
        `Could not delete the temp archive ${filePath}: ${describeError(cleanupError)}`,
      );
    });
  }
...
async function resumeVersionDownload(pending: PendingDownload): Promise<void> {
  ...
  log(
    "Recovery",
    `Resuming version install "${pending.name}" (${pending.versionUuid})`,
  );
  try {
    ...
    log(
      "Recovery",
      `Resuming version "${pending.name}" (${pending.versionUuid}) failed: ${describeError(e)}`,
    );
    const reason = e instanceof Error ? e.message : String(e);
    useAppStore
      .getState()
      .setError(`Could not finish installing ${pending.name}: ${reason}`);
  }
}
...
export function resumePendingDownloads(): void {
  ...
  for (const entry of pending) {
    if (entry.type === "mod") {
      resumeModDownload(entry).catch((e) =>
        failRecovery(entry, describeRecoveryFailure(entry, e)),
      );
    } else if (entry.type === "version") {
      resumeVersionDownload(entry).catch((e) => {
        log(
          "Recovery",
          `Unhandled failure resuming "${entry.name}": ${describeError(e)}`,
        );
      ...
      log(
        "Recovery",
        `Dropping pending download "${entry.name}" (${entry.id}): unknown type "${entry.type}"`,
      );
