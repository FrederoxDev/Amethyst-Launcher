...
function rateLimitDetail(response: Response): string | null {
  ...
  if (!Number.isFinite(reset) || reset <= 0)
    return "the GitHub request allowance is used up";
...
export class GithubTools {
  ...
  static async getLatestRelease(
    ...
    if (cached && Date.now() - cached.fetchedAt < CACHE_TTL_MS) {
      log(
        "Github",
        `Latest release of ${repo} answered from cache: ${cached.release.tagName}`,
      );
  ...
  private static async fetchLatestRelease(
    ...
    for (let attempt = 1; attempt <= ATTEMPTS; attempt++) {
      ...
      try {
        ...
        log(
          "Github",
          `Could not read the latest release of ${repo} from ${apiUrl}: ${describeError(error)}`,
        );
        // Waiting out an hour-long allowance is not a retry, it is a hang.
        if (error instanceof RateLimitedError || attempt === ATTEMPTS)
          throw error;
  ...
  private static async readRelease(
    ...
    const response = timeout
      ? await fetchWithTimeout(apiUrl, {}, timeout)
      : await fetch(apiUrl);
    if (!response.ok) {
