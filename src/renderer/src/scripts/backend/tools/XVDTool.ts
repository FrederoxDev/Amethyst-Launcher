...
function redactCik(args: string[]): string[] {
  return args.map((arg, index) =>
    args[index - 1] === "-cikdata" ? `<${arg.length} chars redacted>` : arg,
  );
}
...
export class XVDTool extends ToolArtifact {
  ...
  isSupported(): boolean {
    ...
    if (!supported) {
      log(
        this.name,
        `Not supported on platform '${window.process.platform}', arch '${window.process.arch}'`,
      );
    }
    return supported;
  }
  ...
  protected async findAsset(
    ...
    for (const asset of release.assets) {
      ...
      if (name.includes(platform) && name.includes(arch)) {
        log(
          this.name,
          `Release ${release.tagName} asset '${asset.name}' matches ${platform} ${arch}`,
        );
        return asset;
      }
    }
  ...
  protected onInstalled(context: ToolInstalledContext): Promise<void> {
    ...
    log(
      this.name,
      `${context.action} '${context.version}', marking '${exe}' executable for Linux`,
    );
    return fs.promises.chmod(exe, 0o755).catch((error) => {
      log(
        this.name,
        `chmod 755 on '${exe}' failed: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    });
  }
  ...
  async decryptFile(
    ...
    if (entries.length === 0) {
      log(
        this.name,
        `No CIK keys were supplied for '${inputFile}', so it cannot be decrypted`,
      );
    ...
    for (const [cikUuid, cikData] of entries) {
      ...
      log(
        this.name,
        `Trying CIK ${cikUuid} (${attempt} of ${entries.length}) on '${inputFile}'`,
      );
      try {
        ...
        log(
          this.name,
          `CIK ${cikUuid} did not decrypt '${inputFile}': ${lastError}`,
        );
      }
    }
    ...
    log(
      this.name,
      `None of the ${entries.length} CIK keys decrypted '${inputFile}'`,
    );
    throw new Error(
  ...
  private async runTool(
    ...
    await ProgressBar.runAsync(
      async ({ setStatus, setMessage, setProgress }) => {
        setStatus(status);
        ...
        if (result.spawnError)
          throw new Error(
            `XVDTool could not be started (${result.spawnError}).`,
          );
        if (result.timedOut) {
          throw new Error(
        ...
        if (result.code !== 0)
          throw new Error(
            `XVDTool ended with code ${result.code}. ${result.output}`.trim(),
          );

        log(
          this.name,
          `${executable} finished with code ${result.code} in ${result.durationMs}ms`,
        );
      },
    );
  }
  ...
  async extractFile(
    ...
    log(
      this.name,
      `Extracting '${inputFile}' to '${outputFolder}', checkForUpdates=${checkForUpdates}`,
    );
    ...
    await this.runTool("extracting", xvdtoolExecutable, [
      "-nd",
      "-xf",
      outputFolder,
      inputFile,
    ...
    const written = fs.existsSync(outputFolder)
      ? fs.readdirSync(outputFolder).length
      : 0;
    if (written === 0) {
      log(
        this.name,
      ...
      throw new Error(
        `XVDTool reported success but nothing was written to ${outputFolder}.`,
      );
    }
    log(
      this.name,
      `Extracted '${inputFile}' to '${outputFolder}', which now holds ${written} entries`,
    );
    return null;
  }
}
