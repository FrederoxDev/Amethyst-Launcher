...
const GAME_CONFIG_STAMP = "Generated by Amethyst Launcher";
/** Bumped whenever the template below changes, so builds prepared by an older one are redone. */
const GAME_CONFIG_REVISION = 1;

function gameConfigStamp(): string {
    return `${GAME_CONFIG_STAMP} ${launcherVersion()} revision ${GAME_CONFIG_REVISION}`;
}

type GameConfigState =
    /** Written by the build itself, so it is the build's business and is never touched. */
    | "not-ours"
    /** This launcher's own output, from this build of it. */
    | "current"
    /** This launcher's own output, from a build of it that wrote a different file. */
    | "stale";

function readGameConfigState(configPath: string): GameConfigState {
    let text: string;
    try {
        text = fs.readFileSync(configPath, "utf-8");
    } catch (e) {
        log("VersionFiles", `Could not read ${configPath} to see which build wrote it: ${describe(e)}`);
        return "not-ours";
    }

    if (!text.includes(GAME_CONFIG_STAMP)) return "not-ours";
    return text.includes(gameConfigStamp()) ? "current" : "stale";
}

/**
 * GDK builds refuse to launch without this next to the exe.
 *
 * The file the launcher writes carries the launcher build that wrote it, because "a file is
 * there" is not "a file is right": one written by a launcher build with a bug in this template
 * would otherwise be kept for the life of the install and the fix would never reach it.
 */
function ensureGameConfig(versionPath: string, manifestXml: string, channel: Channel): void {
    ...
    if (fs.existsSync(configPath)) {
        const state = readGameConfigState(configPath);
        if (state !== "stale") {
            log(
                "VersionFiles",
                `MicrosoftGame.Config left as is (${state}): ${configPath} (${describeFile(configPath)})`
            );
            return;
        }
        log(
