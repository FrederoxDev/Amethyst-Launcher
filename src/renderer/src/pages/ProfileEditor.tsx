...
import { launchErrorMessage, launchProfile as doLaunchProfile } from "@renderer/flows/Launch";
import {
...
function useModIcons(modsPath: string): Map<string, string> {
    ...
    return useMemo(() => {
        ...
        for (const mod of allMods) {
            const iconPath = path.join(modsPath, mod.id, "resource_packs", "main_rp", "pack_icon.png");
            if (fs.existsSync(iconPath)) icons.set(mod.id, `amethyst-icon://icon/${encodeURIComponent(iconPath)}`);
        }
        return icons;
    }, [allMods, modsPath]);
}
...
function ModIcon({ iconUrl }: { iconUrl: string | undefined }) {
    return (
        <div className="profile-editor-mod-icon">
            {iconUrl
                ? <img src={iconUrl} width="36" height="36" className="pixelated" style={{ borderRadius: 3 }} alt="" />
...
export function ProfileEditor() {
    ...
    const editingProfile = allProfiles.find(profile => profile.uuid === editingProfileUuid);
    ...
    useEffect(() => {
        ...
        const commit = () => {
            ...
            const stored = state.profiles.find(profile => profile.uuid === editingProfileUuid);
            if (!stored) {
                log("ProfileEditor", `Autosave skipped: no profile with uuid ${editingProfileUuid}`);
                return;
            }

            // Opening the page is not an edit, and profiles.json is rewritten whole.
            const unchanged = stored.name === profileName
            ...
            state.setProfiles(profiles => profiles.map(profile => profile.uuid !== editingProfileUuid ? profile : {
                ...profile,
                name: profileName,
    ...
    const getOrphanedMods = (modNames: string[], excludeProfileUuid: string | null) => {
        return modNames.filter(modName => {
            if (modName.includes("0.0.0-dev")) return false;
            const otherProfilesUsingMod = allProfiles.filter(p =>
                p.uuid !== excludeProfileUuid && p.mods.includes(modName)
            );
    ...
    const removeMod = async (modName: string) => {
        ...
        if (!proceed) return;
        log("ProfileEditor", `Removed mod "${modName}" from profile "${editingProfile?.name ?? editingProfileUuid}"`);
    ...
    const deleteProfile = async () => {
        ...
        if (!profile) {
            log("ProfileEditor", `Delete ignored: no profile with uuid ${editingProfileUuid}`);
            return;
        }
        ...
        try {
            ...
            useAppStore.getState().setError(`Could not delete ${profile.name}: ${userMessage(e)}`);
            return;
        }
        navigate("/");
    };
    ...
    const onPlay = async () => {
        const profile = editingProfile;
        if (!profile) {
            log("ProfileEditor", `Play ignored: no profile with uuid ${editingProfileUuid}`);
    ...
    const openAddContent = async () => {
        ...
        log("ProfileEditor", `Added mod "${result}" to profile "${editingProfile?.name ?? editingProfileUuid}"`);
        setProfileActiveMods([...profileActiveMods, result]);
    };

    const openVersionPicker = async () => {
        const choice = await Popup.ask<VersionChoice | null>(props => <VersionPickerPopup {...props} />);
        if (!choice) {
    ...
    const allModsList = useMemo(() => {
        const runtimeSet = new Set(modStatuses.filter(mod => mod.isRuntime).map(mod => mod.id));
