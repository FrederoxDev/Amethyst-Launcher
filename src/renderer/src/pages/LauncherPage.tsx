...
const ProfileCard = ({
  ...
  return (
    <div
      className={`launcher-profile-card${isSelected ? " selected" : ""}`}
      onClick={onEdit}
    >
      <div className="launcher-profile-card-info">
        <div className="launcher-profile-card-name-row">
          <p className="minecraft-seven launcher-profile-card-name">
            {profile.name}
          </p>
          {runtimeWarning && (
            <div
              className="launcher-profile-card-warning-inline"
              role="img"
              aria-label={runtimeWarning}
            >
              <svg
                width="16"
                height="16"
        ...
        <p className="minecraft-seven launcher-profile-card-version">
          {versionName} &middot; {channelLabel(profile.channel)} &middot;{" "}
          {profileModeLabel}
        </p>
      </div>
      <div
      ...
      >
        <div className="launcher-profile-card-play-wrap">
          ...
          {runtimeWarning && (
            <div className="launcher-profile-card-play-tooltip minecraft-seven">
              {runtimeWarning}
            </div>
          )}
        </div>
...
export function LauncherPage() {
  ...
  const deleteProfile = async (index: number) => {
    ...
    if (!profile) {
      log(
        "LauncherPage",
        `Delete ignored: no profile at index ${index} of ${allProfiles.length}`,
      );
      return;
    }
  ...
  const handleReorder = (targetUuid: string) => {
    ...
    if (
      !currentDragUuid ||
      currentDragUuid === targetUuid ||
      reorderCooldown.current
    )
      return;
  ...
  const runtimeWarnings = useMemo(() => {
    ...
    for (const profile of allProfiles) {
      ...
      if (blocker !== null)
        warnings.set(profile.uuid, describeProblem(blocker));
    }
    return warnings;
    ...
    <div className="launcher-page">
      ...
      >
          ...
          return (
            <div
              ...
              onDragEnd={() => {
                ...
                if (
                  order.length === startOrder.length &&
                  order.every((uuid, i) => uuid === startOrder[i])
                )
                  return;

                log(
                  "LauncherPage",
                  `Profile order saved: ${allProfiles.map((p) => p.name).join(", ")}`,
                );
                saveData();
              }}
            >
              <ProfileCard
                ...
                onEdit={() => {
                  log(
                    "LauncherPage",
                    `Opening the editor for "${profile.name}" (${profile.uuid})`,
                  );
        ...
        <div
          ...
          onClick={async () => {
            ...
            try {
              ...
              log(
                "LauncherPage",
                `Profile creation failed: ${describeError(e)}`,
              );
        ...
        >
          ...
          <p className="minecraft-seven launcher-create-card-text">
            Create a Profile
          </p>
        </div>
      </div>
