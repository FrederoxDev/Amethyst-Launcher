...
const openModsFolder = () => {
  ...
  if (!fs.existsSync(paths.modsPath)) {
    log(
      "ModsPage",
      `Creating the mods folder ${paths.modsPath} before opening it`,
    );
    fs.mkdirSync(paths.modsPath, { recursive: true });
  }

  const opener =
    window.process.platform === "win32" ? "explorer.exe" : "xdg-open";
  ...
  proc.on("error", (error) =>
    log(
      "ModsPage",
      `Could not start ${opener} for ${paths.modsPath}: ${describeError(error)}`,
    ),
  );
  proc.unref();
};
...
export function ModsPage() {
  ...
  const allReports: readonly ValidatedMod[] = useAppStore(
    (state) => state.allMods,
  );
          ...
          {selectedReport.warnings.length > 0 && (
            <ul>
              {selectedReport.warnings.map((err) => (
                <li
                  className="minecraft-seven mods-popup-warning-item"
                  key={err}
                >
                  - {err}
                </li>
