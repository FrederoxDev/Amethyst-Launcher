...
import {
  ...
  ProgressBar,
} from "@renderer/states/ProgressBarStore";
...
function renderParsedLine(
  ...
  return (
    <div key={lineIdx} className="logs-line">
      {p.time && (
        <span className="logs-line-meta">
          {highlightSegment(p.time, lowerQuery, `l${lineIdx}c`)}
        </span>
      )}
      {p.time && " "}
      <span className="logs-line-meta">
        {highlightSegment(p.thread, lowerQuery, `l${lineIdx}t`)}
      </span>{" "}
      <span className="logs-line-meta">
        {highlightSegment(p.mod, lowerQuery, `l${lineIdx}m`)}
      </span>
      {showLevelTag && " "}
      {showLevelTag && (
        <span className={cls}>
          {highlightSegment(`[${p.level}]`, lowerQuery, `l${lineIdx}e`)}
        </span>
      )}
      {p.rest && (
...
function VirtualLogView({ lines, query }: VirtualLogViewProps) {
  ...
  const start = Math.max(
    0,
    Math.floor(scrollTop / LINE_HEIGHT) - OVERSCAN_LINES,
  );
  const end = Math.min(
    lines.length,
    Math.ceil((scrollTop + viewportH) / LINE_HEIGHT) + OVERSCAN_LINES,
  );
    ...
    <div className="logs-filter-root" ref={rootRef}>
      ...
      <div className="logs-filter-trigger-wrap">
        <div className="logs-filter-trigger" onClick={() => setOpen((o) => !o)}>
          <span className="minecraft-seven logs-filter-trigger-text">
            {summary}
          </span>
        ...
        {open && (
          <div className="logs-filter-panel">
            {options.length === 0 && (
              <p className="minecraft-seven logs-filter-empty">No values</p>
            )}
            {options.map((opt) => {
              ...
              return (
                ...
                >
                  <div
                    className={`logs-filter-checkbox${checked ? " checked" : ""}`}
                  >
                    {checked && (
                  ...
                  <span className="minecraft-seven logs-filter-option-text">
                    {opt}
                  </span>
                </div>
              );
...
export function LogsPage() {
  ...
  const logsDir = useMemo(
    () => path.join(platform.getPaths().amethystPath, "Launcher", "Logs"),
    [platform],
  );
  ...
  const copyAsPath = async (file: LogFile) => {
    try {
      ...
      log(
        "LogsPage",
        `Could not copy ${file.path} to the clipboard: ${describeError(e)}`,
      );
  ...
  const showInExplorer = (file: LogFile) => {
    try {
      ...
      log(
        "LogsPage",
        `Could not show ${file.path} in the file manager: ${describeError(e)}`,
      );
      setError(
        `Could not show ${file.name} in the file manager: ${userMessage(e)}`,
      );
    }
    setContextMenu(null);
  };
        ...
        await Promise.all(
          entries.map(async (name) => {
            ...
            try {
              ...
              log(
                "LogsPage",
                `Leaving ${full} out of the list: ${describeError(e)}`,
              );
              return null;
            }
          }),
  ...
  useEffect(() => {
    ...
    readLogTail(selected)
      ...
      .catch((e) => {
        ...
        setLines([]);
        setError(
          `Could not read ${path.basename(selected)}: ${userMessage(e)}`,
        );
      });
    return () => {
      cancelled = true;
  ...
  const deleteAllLogs = async () => {
    ...
    const deletable = files.filter(
      (f) => path.resolve(f.path) !== path.resolve(current),
    );

    if (deletable.length === 0) {
      log(
        "LogsPage",
        `Delete All ignored: ${logsDir} holds nothing but the run in progress`,
      );
      return;
    }
    const ok = await confirmAction({
    ...
    if (!ok) {
      log(
        "LogsPage",
        `Deletion of ${deletable.length} log(s) cancelled by the user`,
      );
      return;
    }
    try {
      await ProgressBar.runAsync(
        async (state) => {
          ...
          log(
            "LogsPage",
            `Deleting ${deletable.length} log(s) in ${logsDir}, keeping the run in progress`,
          );
          await Promise.all(
            deletable.map((f) =>
              fs.promises.unlink(f.path).catch((e) => {
                ...
                log(
                  "LogsPage",
                  `Could not delete ${f.path}: ${describeError(e)}`,
                );
              }),
            ),
          );
  ...
  const openLogsFolder = async () => {
    try {
      ...
      log(
        "LogsPage",
        openError
          ? `Could not open ${logsDir}: ${openError}`
          : `Opened ${logsDir}`,
      );
    } catch (e) {
  ...
  const copySelected = async () => {
    ...
    try {
      await navigator.clipboard.writeText(
        await fs.promises.readFile(selected, "utf-8"),
      );
      log("LogsPage", `Copied ${selected} to the clipboard`);
    } catch (e) {
      log(
        "LogsPage",
        `Could not copy ${selected} to the clipboard: ${describeError(e)}`,
      );
  ...
  const filteredLines = useMemo(() => {
    ...
    return lines.filter((p) => {
      if (threadFilter.size > 0 && (!p.thread || !threadFilter.has(p.thread)))
        return false;
  ...
  return (
    <div className="logs-page">
            ...
            <>
              <div className="logs-viewer-header">
                <div className="logs-viewer-name-wrap">
                  <p className="minecraft-seven logs-viewer-name">
                    {path.basename(selected)}
                  </p>
                  {(() => {
                    const f = files.find((x) => x.path === selected);
                    return f ? (
                      <p className="minecraft-seven logs-viewer-size">
                        {formatSize(f.size)}
                      </p>
                    ) : null;
                  })()}
              ...
              {truncatedFrom !== null && (
                <p className="minecraft-seven logs-viewer-truncated">
                  Showing the last {formatSize(MAX_VIEW_BYTES)} of{" "}
                  {formatSize(truncatedFrom)}. Copy or open the file for the
                  rest.
                </p>
              )}
      ...
      {contextMenu && (
        ...
        >
          <div
            className="logs-context-menu-item"
            onClick={() => showInExplorer(contextMenu.file)}
          >
            <p className="minecraft-seven">Open in Explorer</p>
          </div>
          <div
            className="logs-context-menu-item"
            onClick={() => copyAsPath(contextMenu.file)}
          >
