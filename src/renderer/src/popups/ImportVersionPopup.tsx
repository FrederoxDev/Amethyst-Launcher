...
export function ImportVersionPopup({ submit, initialFile, defaultChannel, onBack }: ImportVersionPopupProps) {
    ...
    const detected = useMemo(() => {
        ...
        return {
            version: prettifyVersionFromFilename(name) ?? name.match(/\d+\.\d+\.\d+\.\d+/)?.[0] ?? null,
            channel: channelFromFilename(name),
        };
    }, [file]);

    const versionText = versionOverride ?? detected?.version ?? "";
    const channel = channelOverride ?? detected?.channel ?? defaultChannel ?? "release";
    const label = labelOverride ?? `Minecraft ${versionText} (${channelLabel(channel)})`;
    ...
    return (
        ...
        >
            ...
            <Dropdown
                ...
                setValue={value => setChannelOverride(parseChannel(value) ?? "release")}
            />

            <TextInput label="Version" text={versionText} setText={setVersionOverride} style={{ width: "100%" }} />
