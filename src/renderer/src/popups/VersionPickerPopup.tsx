...
export function VersionPickerPopup({
  submit: rawSubmit,
  state,
  restrictToChannel,
}: Props) {
  const animateClose = usePopupClose();
  const submit = (result: VersionChoice | null) =>
    animateClose(() => rawSubmit(result));
  ...
  const [showPreviews, setShowPreviews] = useState(
    () =>
      restrictToChannel === "preview" ||
      localStorage.getItem("version-picker-show-previews") === "true",
  );
  ...
  const visibleInstalled = useMemo(
    () =>
      installedVersions.filter(
        (v) => !restrictToChannel || v.channel === restrictToChannel,
      ),
  ...
  return (
    <PopupPanel
      title={
        restrictToChannel
          ? `Select ${channelLabel(restrictToChannel)} Version`
          : "Select Version"
      }
      onClose={() => submit(null)}
      size="xl"
    ...
    >
      {visibleInstalled.length > 0 && (
        <>
          <p className="minecraft-seven version-picker-section-title">
            Installed
          </p>
      ...
      {fetching
        ? Array.from({ length: 8 }).map((_, i) => (
            <div
              key={i}
              className="version-picker-item version-picker-skeleton"
            >
              <div
                className="version-picker-skeleton-text"
